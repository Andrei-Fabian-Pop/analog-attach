...
async function test_impl(data: BindingTestData) {
    ...
    if (data.debug === true) {
        write_to_directory(
            path.resolve(__dirname, 'expected/canary-inserter'),
            data.name,
            canary_binding
        );
    }

    const expected_path = path.resolve(__dirname, `expected/canary-inserter/${data.name}.json`);
    const expected = JSON.stringify(JSON.parse(fs.readFileSync(expected_path, { encoding: 'utf8' })));

    expect(JSON.stringify(canary_binding)).toStrictEqual(expected);
}