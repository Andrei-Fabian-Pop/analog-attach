...
describe('Property Resolver Test', () => {
...
async function test_impl(data: BindingTestData) {
    ...
    if (data.debug === true) {
        write_to_directory(
            path.resolve(__dirname, 'expected/property-resolver'),
            data.name,
            property_resolved);
    }

    const expected_path = path.resolve(__dirname, `expected/property-resolver/${data.name}.json`);
    const expected = JSON.stringify(JSON.parse(fs.readFileSync(expected_path, { encoding: 'utf8' })));

    expect(JSON.stringify(property_resolved)).toStrictEqual(expected);
}