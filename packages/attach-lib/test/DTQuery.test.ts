...
import { Attach, parse_dts, query_devicetree, suggest_parents } from 'attach-lib';
...
async function test_impl(data: BindingTestData) {
    ...
    if (data.debug === true) {
        write_to_directory(
            path.resolve(__dirname, "expected/dts-query"),
            data.name,
            binding.parsed_binding
        );
    }

    const expected_path = path.resolve(__dirname, `expected/dts-query/${data.name}.json`);
