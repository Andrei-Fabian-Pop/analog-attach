...
describe('Redefinition Merger Test', () => {
...
async function test_impl(data: BindingTestData) {
    ...
    if (data.debug === true) {
        write_to_directory(
            path.resolve(__dirname, "expected/redefinition-merger"),
            data.name,
            redefinition_merged
        );
    }

    const expected_path = path.resolve(__dirname, `expected/redefinition-merger/${data.name}.json`);
    const expected = JSON.stringify(JSON.parse(fs.readFileSync(expected_path, { encoding: 'utf8' })));

    expect(JSON.stringify(redefinition_merged)).toStrictEqual(expected);
}
